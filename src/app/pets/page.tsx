...
// Palette of distinct, vivid gradients for pet avatars
const COLOR_PALETTE = [
  { key: 'rose',    from: '#f43f5e', to: '#e11d48', label: 'Rose' },
  { key: 'violet',  from: '#8b5cf6', to: '#7c3aed', label: 'Violet' },
  { key: 'sky',     from: '#0ea5e9', to: '#0284c7', label: 'Bleu' },
  { key: 'amber',   from: '#f59e0b', to: '#d97706', label: 'Ambre' },
  { key: 'emerald', from: '#10b981', to: '#059669', label: 'Vert' },
  { key: 'orange',  from: '#f97316', to: '#ea580c', label: 'Orange' },
  { key: 'indigo',  from: '#6366f1', to: '#4f46e5', label: 'Indigo' },
  { key: 'pink',    from: '#ec4899', to: '#db2777', label: 'Rose vif' },
]
      ...
      <main className="mx-auto max-w-5xl px-4 pt-8 md:px-8 md:pt-32 relative z-10">
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-1 fade-in-up stagger-2">
            <div className="rounded-[2rem] glass-card p-6 md:p-8 lg:sticky lg:top-24 space-y-8">
              ...
              <div>
                ...
                <form action={async (formData) => { "use server"; await createPet(formData) }} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-zinc-600">Couleur de l'avatar</label>
                    <div className="grid grid-cols-4 gap-2">
                      {COLOR_PALETTE.map((c, i) => (
                        <label key={c.key} className="cursor-pointer">
                          ...
                          />
                          <div
                            className="h-10 w-full rounded-xl shadow-sm ring-2 ring-transparent peer-checked:ring-offset-2 peer-checked:ring-zinc-900 transition-all hover:scale-105"
                    ...
                    <div className="p-5 md:p-7">
                      <div className="flex items-start gap-4">
                        ...
                        <div
                          ...
                          style={{ background: `linear-gradient(135deg, ${col.from}, ${col.to})`, boxShadow: `0 8px 20px -4px ${col.from}60` }}
                        >
                          {initial}
                        </div>
                            ...
                            {birthdayInfo && (
                              <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-bold ${
                                ...
                                {birthdayInfo.daysUntil === 0
                                  ...
                                  ? '🎂 Anniversaire demain !'
                                  : `🎂 Anniversaire dans ${birthdayInfo.daysUntil}j`}
                              </span>
                            )}
                          </div>
                        </div>
                        ...
                        {/* Delete pet */}
                        <form action={async () => { "use server"; await deletePet(pet.id) }} className="shrink-0 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                          ...
                          {treatments.map((t: any) => {
                            ...
                            return (
                              <div key={t.id} className={`rounded-2xl border p-4 transition-colors ${
                                isEmpty ? 'border-red-200 bg-red-50/60'
                                : isLow ? 'border-amber-200 bg-amber-50/60'
                                ...
                                <div className="flex flex-wrap items-center gap-2 mb-3">
                                  ...
                                  {isEmpty && (
                                    <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-black uppercase tracking-wider text-red-600">
                                      <AlertTriangle className="h-3 w-3" /> Vide
                                    </span>
                                  )}
                                  {!isEmpty && isLow && (
                                    <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-black uppercase tracking-wider text-amber-600">
                                      <AlertTriangle className="h-3 w-3" /> Stock faible
                                    </span>
                                  )}
                                  {isDue && !isEmpty && (
                                    <span className="inline-flex items-center gap-1 rounded-full bg-indigo-100 px-2 py-0.5 text-[10px] font-black uppercase tracking-wider text-indigo-600">
                                      À donner
                                    </span>
                                  )}
                                </div>
                                ...
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  <form action={async (fd) => {
                                    ...
                                    <input
                                      ...
                                      min="1"
                                      defaultValue="6"
                                      className="w-14 rounded-lg border border-zinc-200 bg-white px-2 py-1 text-center text-xs font-bold text-zinc-700 focus:outline-none focus:border-zinc-300"
                                    />
                                    <button type="submit" title="Réapprovisionner" className="flex h-9 items-center gap-1.5 rounded-xl border border-zinc-200 bg-white px-3 text-xs font-bold text-zinc-500 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200 transition-colors">
